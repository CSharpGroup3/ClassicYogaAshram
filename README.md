...
**Attendance- таблица (Посещаемость занятии) поля :**
...
**AttendanceCount- таблица (Вспомогательная таблица для подсчета  текущего  посещения клиента) поля :**
...
**Membership- таблица (Описывает модель абонемента) поля :**
...
**ClientsMembership- таблица (Для хранения данных о покупке клиента определенного абонемента) поля :**
...
**Scheduele- таблица (Описывает модель расписания группы) поля :**
...
- StartBot - запуск бота напоминания Админу что есть не отмеченные клиенты.
...
супер админ с ролем Chief. Chief логируется в приложения дальше сразу должен создать работников. И так после создания работников создается Администратору 
Филиалы после создания филиалов нужно каждому филиалу создать группы у группы, есть клинты также создаются клиенты.<br> 
Дальше у Админастраций сайта, есть клиенты .Клиенты могут купить абонименты у абониментов есть, дата и сумма. После приобритения Абонимента клиент полноценно 
...
или же отмены занятий переходить на следующий день без потеря преведущего дня занятий.<br>
